function [fitVal,varargout] = mypsotestfunc(xVec,params)
...
%
%[F,R] =  MYPSOTESTFUNC(X,P)
%returns the real coordinates in R. 
%
%[F,R,Xp] = MYPSOTESTFUNC(X,P)
...
for lpc = 1:nrows
    ...
    % functions
        x = xVec(lpc,:);
        fitVal(lpc) = (1/4000)*sum(x.^2) - prod(cos(x ./ sqrt(1:numel(x)))) + 1; % Generalised Griewank
    end
end
