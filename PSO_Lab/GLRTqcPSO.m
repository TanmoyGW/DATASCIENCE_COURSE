...
% 'npsd': PSD values at positive DFT frequencies
%The fields of O are:
...
for lpruns = 1:nRuns   
    ...
    estSig = QuadChirpDTS(inParams.dataX,1,qcCoefs);
    estSig = normsig4psd(estSig,inParams.sf,inParams.npsd,1); % For colored noise
    % estAmp = inParams.dataY*estSig(:);
    estAmp = innerprodpsd(estSig,inParams.dataY,inParams.sf,inParams.npsd);
